...
if __name__ == "__main__":
    data = []
    score = []
    pools = fetch_pools(limit=100)
    for p in pools:
        # apy_search(p["pool"])
        print("Pool:")
        print(p)

        a = score_defillama_pool(p)
        data.append(a)
        score.append(a["final_score"])

    # Find the pool with the highest risk score
    highest_score = max(data, key=lambda x: float(x["final_score"]))
    print("Pool with highest score:")
    print(json.dumps(highest_score, indent=2))